...
class RestApi(http.Controller):
    ...
    def odoo_connect(self, **kw):
        ...
        try:
            ...
            # Compatibility for Odoo 17 vs Odoo 18 authentication
            try:
                # Odoo 17: authenticate(db, login, password) -> returns uid
                auth = request.session.authenticate(request.session.db, username, password)
            except TypeError:
                # Odoo 18: authenticate(db, credential_dict) -> returns dict
                _logger.info("Antigravity: Detected Odoo 18 authentication signature")
                auth_info = request.session.authenticate(request.session.db, {
                    'login': username, 
                    'password': password,
                    'type': 'password'
                })
            ...
            _logger.error("Antigravity: Authentication failed: %s", str(e))
