...
class HrLoan(models.Model):
    ...
    def _compute_loan_amount(self):
        total_paid = 0.0
        for loan in self:
            for line in loan.loan_lines.filtered(lambda x: x.paid):
    ...
    def _get_journal_count(self):
        for rec in self:
            journal_ids = self.env['account.move'].search([('loan_id', '=', rec.id)])
            rec.journal_count = len(journal_ids.ids)

    def action_journal_entries(self):
        journal_ids = self.env['account.move'].search([('loan_id', '=', self.id)])
        return {
    ...
    journal_count = fields.Integer('Journal Count', compute="_get_journal_count")
    # name = fields.Char(string="Loan Name", default="/", readonly=True, help="Name of the loan")
    name = fields.Char(string="Loan Name", default=lambda self: _('New'), copy=False, readonly=True, tracking=True)
    ...
    def create(self, values):
        if values.get('types') == 'loan':
            values['name'] = self.env['ir.sequence'].next_by_code('loan.seq') or '/'
        if values.get('types') == 'salary_advance':
            values['name'] = self.env['ir.sequence'].next_by_code('loan.advance.seq') or '/'
        loan_count = self.env['hr.loan'].search_count(
            [('employee_id', '=', values['employee_id']), ('state', '=', 'approve'),
             ('balance_amount', '!=', 0), ('types', '=', 'loan')])
        salary_count = self.env['hr.loan'].search_count(
            [('employee_id', '=', values['employee_id']), ('state', '=', 'approve'),
             ('balance_amount', '!=', 0), ('types', '=', 'salary_advance')])
        if salary_count > 1:
            raise ValidationError(_("The employee has already a pending salary installment"))
        if loan_count > 1:
...
class HrEmployee(models.Model):
    ...
    def _compute_employee_loans(self):
        for loan in self:
            ...
            loan.loan_count = self.env['hr.loan'].search_count([('employee_id', '=', loan.id)])
...
class Company(models.Model):
    ...
    credit_account_id = fields.Many2one('account.account', string="Loan Credit Account")
    debit_account_id = fields.Many2one('account.account', string="Loan Debit Account")
